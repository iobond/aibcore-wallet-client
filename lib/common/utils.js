...
var PrivateKey = Aibcore.PrivateKey;
var PublicKey = Aibcore.PublicKey;
var crypto = Aibcore.crypto;
var encoding = Aibcore.encoding;
...
Utils.hashMessage = function(text) {
  ...
  ret = new Aibcore.encoding.BufferReader(ret).readReverse();
  return ret;
};
...
Utils.privateKeyToAESKey = function(privKey) {
  ...
  $.checkArgument(Aibcore.PrivateKey.isValid(privKey), 'The private key received is invalid');
  var pk = Aibcore.PrivateKey.fromString(privKey);
  return Aibcore.crypto.Hash.sha256(pk.toBuffer()).slice(0, 16).toString('base64');
};
...
Utils.deriveAddress = function(scriptType, publicKeyRing, path, m, network, coin) {
  ...
  coin = coin || 'AIB';
  var bitcore = Bitcore_[coin] || Aibcore;
...
Utils.xPubToCopayerId = function(coin, xpub) {
  //AIB Upgrade Critical changes for xpub key(copayerId) generate
  var str;
  if ( coin == Constants.COINS.AIB || coin == Constants.COINS.BTC || coin.length>0) {
    str = xpub;
  } else {
    str = coin + xpub;
  }
...
Utils.signRequestPubKey = function(requestPubKey, xPrivKey) {
  var priv = new Aibcore.HDPrivateKey(xPrivKey).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).privateKey;
  return Utils.signMessage(requestPubKey, priv);
};

Utils.verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new Aibcore.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
Utils.buildTx = function(txp) {
  var coin = txp.coin || Constants.COINS.AIB;
  ...
  if (txp.nullData) {
    if (_.isString(txp.nullData)){ // && JSUtil.isHexa(self.nullData)) {
      t.addData(new buffer.Buffer(txp.nullData, 'hex'));
    }
  }
