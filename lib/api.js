...
API.prototype.createSellTxProposal = function(opts, cb) {
  ...
  self._doPostRequest('/v1/seller/getxp/', args, function(err, orderTxp) {
    if (err) return cb(err);

    self._processTxps(orderTxp.txp);
    if (!Verifier.checkProposalCreation(args, orderTxp.txp, self.credentials.sharedEncryptingKey)) {
      return cb(new Errors.SERVER_COMPROMISED);
    }

    return cb(null, orderTxp);
  });
};
