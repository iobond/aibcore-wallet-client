...
var Aibcore = require('aibcore-lib');
var Bitcore_ = {
  aib: Aibcore,
  btc: Bitcore,
...
API.prototype.seedFromRandom = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.create(opts.coin || Constants.COINS.AIB, opts.network || 'livenet',opts.walletTypes);
};
...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpub = Aibcore.HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys() {
    ...
    if (!xpriv) {
      xpriv = new Aibcore.HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
    var xpub = new Aibcore.HDPublicKey(c.xPubKey);
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.createWithMnemonic(opts.coin || Constants.COINS.AIB, opts.network || 'livenet', opts.passphrase, opts.language || 'en', opts.account || 0, opts);
};
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  ...
  var privateKey = new Aibcore.PrivateKey(privateKeyWif);
  var address = privateKey.publicKey.toAddress().toString();
  var addrBuff = new Buffer(address, 'ascii');
  var actualChecksum = Aibcore.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
  var expectedChecksum = Aibcore.encoding.Base58Check.decode(encryptedPrivateKeyBase58).toString('hex').substring(6, 14);
...
API._buildSecret = function(walletId, walletPrivKey, coin, network) {
  if (_.isString(walletPrivKey)) {
    walletPrivKey = Aibcore.PrivateKey.fromString(walletPrivKey);
  }
  var widHex = new Buffer(walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new Aibcore.encoding.Base58(widHex).toString();
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    var widHex = Aibcore.encoding.Base58.decode(widBase58).toString('hex');
    var walletId = split(widHex, [8, 12, 16, 20]).join('-');

    var walletPrivKey = Aibcore.PrivateKey.fromString(secretSplit[1]);
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Aibcore.HDPrivateKey(derivedXPrivKey);
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'btc';
  if (!_.includes(opts.walletTypes, coin.toUpperCase())) return cb(new Error('Invalid coin'));
  ...
  var defaultPrivateKey = new Aibcore.PrivateKey();
  switch (coin) {
    case Constants.COINS.BTC:
      defaultPrivateKey = new Bitcore.PrivateKey();
      break;
    case Constants.COINS.BCH:
      defaultPrivateKey = new Bitcore_.bch.PrivateKey();
      break;
    default:
      defaultPrivateKey = new Aibcore.PrivateKey();
  }

  var walletPrivKey = opts.walletPrivKey || defaultPrivateKey;
  ...
  var args = {
    ...
    m: m,
    n: n,
    pubKey: (new Aibcore.PrivateKey(walletPrivKey)).toPublicKey().toString(),
    coin: coin,
    network: network,
...
API.prototype.joinWallet = function(secret, copayerName, opts, cb) {
  ...
  var coin = opts.coin || 'btc';
  if (!_.includes([Constants.COINS.BTC, Constants.COINS.BCH, Constants.COINS.AIB], coin)) return cb(new Error('Invalid coin'));
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var walletPrivKey = Aibcore.PrivateKey.fromString(c.walletPrivKey);
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes([Constants.COINS.BTC, Constants.COINS.BCH, Constants.COINS.AIB], opts.coin)) return cb(new Error('Invalid coin'));
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new Aibcore.PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new Aibcore.HDPrivateKey(this.credentials.xPrivKey)
 ...
 * @return {undefined}
 */
API.prototype.getWalletTypes = function(opts, cb) {
  var self = this;

  opts = opts || {};
  var limit = opts.limit || 20;
  var url = '/v1/tokens/' + limit;
 ...
 * @returns {Callback} cb - Return error or the transaction proposal
 */
API.prototype.createSellTxProposal = function(opts, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  $.checkState(this.credentials.sharedEncryptingKey);
  $.checkArgument(opts);
  ...
  self._doPostRequest('/v2/sellerGetTxp/', args, function(err, txp) {
    if (err) return cb(err);
  ...
  var txp = {"toAddress":"AYXNz68Y9ahkEKSkgNa5uXHQcqN2jJpLx6","sendMax":false,"amount":10000000,"description":"E-Transfer to dev@aib.one","spendUnconfirmed":false,"recipientType":"wallet","name":"bch","network":"livenet","coin":"9CAT","txp":{"95c856d1-9436-45fd-a579-5095fc274c86":{"version":3,"createdOn":1544489601,"id":"e2bd25c8-12a5-4305-8e20-6f8b4e48c6db","walletId":"95c856d1-9436-45fd-a579-5095fc274c86","creatorId":"0fce6daa668b5e7fcbc639727743e86d479d706a906a9d348778dbb8082f8b6f","message":"1232354","payProUrl":null,"changeAddress":{"version":"1.0.0","createdOn":1544489601,"address":"AcN1ewqcU3XwBrE5FBnbbC7Lh24EV9rrts","walletId":"95c856d1-9436-45fd-a579-5095fc274c86","isChange":true,"path":"m/1/3","publicKeys":["02b5a8e96faaf2c077f8a0b8914109646be717dbeb99cf5efbb78b78566984b53f"],"network":"livenet","type":"P2PKH"},"outputs":[{"amount":1000000,"toAddress":"AYXNz68Y9ahkEKSkgNa5uXHQcqN2jJpLx6","message":"1232354","encryptedMessage":"{\"iv\":\"1GFiPFpOiqY1ODrmbWvM5w==\",\"v\":1,\"iter\":1,\"ks\":128,\"ts\":64,\"mode\":\"ccm\",\"adata\":\"\",\"cipher\":\"aes\",\"ct\":\"16LJsWd1vBA0VYQhTQmb\"}"}],"outputOrder":[0,1,2],"walletM":1,"walletN":1,"requiredSignatures":1,"requiredRejections":1,"status":"temporary","actions":[],"feeLevel":"normal","feePerKb":225000,"excludeUnconfirmedUtxos":true,"addressType":"P2PKH","customData":{"toWalletName":"bch"},"nullData":"6c0139434154000a3943415420546f6b656e","amount":1000000,"network":"livenet","inputs":[{"txid":"d3e0e3cd9099ef2b8e86835913dd2cc923d8ac373a264bf8ddf88f5441357869","vout":2,"address":"ATGW6Di3xVQqKfhTQjrHsja8uUG8Wbdf9R","scriptPubKey":"76a9147e1373b87165b971425cd81f7ecb1c12c64ff81c88ac","satoshis":48790650,"confirmations":25,"locked":false,"path":"m/1/2","publicKeys":["02c64e04479e6db1f8c6ba550809f3eb60cdbab0d93d4083477fa8d13867551ff8"]}],"inputPaths":["m/1/2"],"fee":55555,"encryptedMessage":"{\"iv\":\"jnTHfSSERWq+miQinjYJxQ==\",\"v\":1,\"iter\":1,\"ks\":128,\"ts\":64,\"mode\":\"ccm\",\"adata\":\"\",\"cipher\":\"aes\",\"ct\":\"/NQPmV6Qu89RMiMSgMmi\"}","creatorName":"","hasUnconfirmedInputs":false,"feeRatePerStr":"5.18%","feeTooHigh":false}},"origToAddress":"AYXNz68Y9ahkEKSkgNa5uXHQcqN2jJpLx6","feeLevel":"normal","feeLevelName":"Normal","feeRate":225000}
  return cb(null, txp);
};
 ...
 * @returns {Callback} cb - Return error or null
 */
/* API.prototype.publishSellTxProposal = function(opts, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  $.checkArgument(opts)
    .checkArgument(opts.txp);
  ...
  var url = '/v1/sellerPublishTxp/' + opts.txp.id + '/publish/';
  self._doPostRequest(url, args, function(err, txp) {
    if (err) return cb(err);
    self._processTxps(txp);
    return cb(null, txp);
