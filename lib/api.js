...
/**
 * Verify Token Origin
 * @param {tokenid: string, txid: string}
 */
/* API.prototype.verifyToken = function(args, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  var self = this;
  var url = '/v1/token/verify/';
  self._doPostRequest(url, args, function(err, result) {
    if (err) return cb(err);
    return cb(null, result);
  });
}; */
 ...
 * @param {tokenid: string, txid: string}
 */
API.prototype.verifyToken = function(args) {
  $.checkState(this.credentials && this.credentials.isComplete());
  var self = this;
  var url = '/v1/token/verify/';
  self._doPostRequest(url, args, function(err, result) {
