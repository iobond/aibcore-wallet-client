...
var Bitcore = require('aibcore-lib');
...
function _checkCoin(coin,walletTypes) {
  if (!_.includes(walletTypes, coin.toUpperCase())) throw new Error('Invalid coin');
  //if (!_.includes([Constants.COINS.BTC, Constants.COINS.BCH, Constants.COINS.AIB], coin)) throw new Error('Invalid coin');
};
...
Credentials.create = function(coin, network,walletTypes) {
  _checkCoin(coin,walletTypes);
...
Credentials.createWithMnemonic = function(coin, network, passphrase, language, account, opts) {
  _checkCoin(coin,opts.walletTypes);
  _checkNetwork(network);
...
Credentials.fromObj = function(obj) {
  ...
  x.coin = x.coin || Constants.COINS.BTC;
...
Credentials.fromOldCopayWallet = function(w) {
  ...
  credentials.coin = Constants.COINS.BTC;
